...
version 1.6.0:
	- added lemmatization in the ngram
	- 
"""
...
headers = pickle.load(open("resource/header_0.4.pickle","rb"))
...
def ngram(doc, number, connect = "__"):
	...
	for token in doc: #iterate the tokenized text (i.e., Spacy doc obj)
		...
		if token.i + number > last_index: #if the next index doesn't exist (because it is larger than the last index)
			...
			if ngram_lemm_string not in ngram_dict: # create an entry in ngram dict
				ngram_dict[ngram_lemm_string] = {'type': ngram_string,
											'lemma': ngram_lemm_string,
											'loc': [ngram_id],
											'Occurrence': 1}
			else:
				ngram_dict[ngram_lemm_string]['loc'].append(ngram_id) #for the second occurrence (and later) we only need the token id info
				ngram_dict[ngram_lemm_string]['Occurrence'] += 1 #for the second occurrence (and later) we only need the token id info
...
def mwu_search2(matched_mwu: dict,text_mwu: dict, mwu_dict:dict) -> dict:
	...
	for mwu_lemma, mwu_info in text_mwu.items(): #iterate the text_mwu
		mwu = mwu_lemma.lower() #
		if (mwu not in mwu_dict):
			if (mwu_info['type'].lower() not in mwu_dict):
				continue
			else:
				mwu = mwu_info['type'].lower()
		...
		if mwu not in matched_mwu[mwu_dict_name]: #see if the mwu is identified using other resources
			...
			matched_mwu[mwu_dict_name][mwu].update(text_mwu[mwu_lemma]) 
