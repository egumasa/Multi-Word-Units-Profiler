...
def mwu_search2(matched_mwu: dict,text_mwu: dict, mwu_dict:dict) -> dict:
	...
	for mwu in text_mwu: #iterate the text_mwu
		if mwu not in mwu_dict: #if it is not in the selected list, igore
			...
			if mwu not in matched_mwu[mwu_dict_name]: #see if the matched_mwu alredy has an entry, if not create one
				matched_mwu[mwu_dict_name][mwu] = {}
				matched_mwu[mwu_dict_name][mwu].update(text_mwu[mwu])
...
def update_token_info(matched_mwus, token_info):
	for mwu, dict_mwu in matched_mwus.items():
		...
		if len(dict_mwu['Biber_2004']) > 0:
			color = color_biber(dict_mwu['Biber_2004']['Frequency in Conversation'], color_codes)
			overwrite_color(token_loc, color, token_info)
...
def update_token_info2(matched_mwus, token_info):
	...
	if len(matched_mwus['PHRASAL']) > 0:
		...
		for mwu, item_info in matched_mwus['PHRASAL'].items():
			token_loc = item_info["loc"]
			color = color_phrasal(item_info['Frequency Level'], color_codes)
			overwrite_color(token_loc, color, token_info)

	if len(matched_mwus['AFL_all']) > 0:
		for mwu, item_info in matched_mwus['AFL_all'].items():
			token_loc = item_info["loc"]
			color = color_AFL(item_info['Sub-list'], color_codes)
			overwrite_color(token_loc, color, token_info)
