...
def dict_freq(mwu_dict, matched_list, header):
	holder = {}
	...
	if request.method == 'POST': #when text input is given
		...
		if input_text:
			...
			if selected_list == "AFL_all":
				header = AFL_head_s
				example = dict_freq(AFL_all, pattern_matching(AFL_all, input_text), header)
			elif selected_list == "PHRASAL":
				header = PHRASAL_head_s
				example = dict_freq(PHRASAL, pattern_matching(PHRASAL, input_text), header)
			
			elif selected_list == "Biber_2004": 
				header = Biber_header_s
				example = dict_freq(Biber_2004, pattern_matching(Biber_2004, input_text), header)
